    ...
    def get_alloc_top_age_reg(self, result_df, outcome_metric):
        ...
        for i in range(len(top_reg)):
            for j in range(len(top_age)):
                alloc = np.array(top_reg.iloc[i].alloc)/sum(top_reg.iloc[i].alloc) * np.array(top_age.iloc[j].alloc)/sum(top_age.iloc[j].alloc)
        ...
        for i in range(num_group):
            ...
            alloc[i] = sum(top_reg.iloc[0].alloc)
            alloc_list.append(alloc)
...
def organize_df(df, sort_by='cost_deaths_0'):
...
#     for p in [i for i in range(5)]:
#         B = [200,10000][fips_list.index(fips_num)]
#         result_file_path = f'Result/{fips_num}_obj_all_alg_reg_age_B_{B}_point_{p}_date_0920'
#         result_df = pd.read_pickle(result_file_path+'.pkl')
#         outcome_list = [col for col in result_df.columns if col in ['cost_deaths_0','disparity_deaths_0','cost_vacc_0','disparity_vacc_0']]
...
#     global_top_df.to_csv('top_results_final_1025.csv')
# global_top_df.to_pickle('top_results_final_1025.pkl')
